...
int AlignCheckCommand::execute(){
	try {
		...
		m->mothurOutEndLine();
		m->mothurOut("\t\tPound\tDash\tPlus\tEqual\tLoop\tTilde\tTotal\n");
		...
		else { m->mothurOut("# of unique seqs:\t" + toString(count)); m->mothurOut("\ntotal # of seqs:\t" + toString(size)); m->mothurOutEndLine(); }
